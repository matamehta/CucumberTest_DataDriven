...
import info.seleniumcucumber.methods.RetriveTestdata;
...
public class UserStepDefinitions implements BaseTest{
    ...
    RetriveTestdata retriveTestdata =new RetriveTestdata();
...
// Input steps -------------
    ...
    public void enter_text(String text,String field, String type,String accessName) throws Exception
    {
        miscmethodObj.validateLocator(type);
        String name= retriveTestdata.YamlFileReader().get(text).get(field);
        inputObj.enterText(type, name, accessName);
    }
...
// Assertion method   ---------------
    ...
    @Then("^element having (.+)\"([^\"]*)\" from \"([^\"]*)\"  should\\s*((?:not)?)\\s+be present$")
    public void elementHavingXpathShouldFromBePresent(String type,String accessName,String text,String present) throws Throwable {
        miscmethodObj.validateLocator(type);

        String name= retriveTestdata.YamlFileReader().get(text).get(accessName);
        assertionObj.checkElementPresence(type, name, present.isEmpty());
    }
    ...
    @Then("^I assert repeater element having \"([^\"]*)\" should have \"([^\"]*)\" from \"([^\"]*)\" have text present$")
    public void i_assert_repeater_element_having_should_have_text_present(String accessName, String value,String data) throws Throwable {
        // Write code here that turns the phrase above into concrete actions
        String data_value =retriveTestdata.YamlFileReader().get(data).get(value);
        assertionObj.checkRepeaterElementText(accessName,data_value);
    }

    // step to click on element from repeater list
    @Then("^I click on repeater element having text \"([^\"]*)\" should have \"([^\"]*)\" from  \"([^\"]*)\" to be clicked$")
    public void i_click_on_repeater_element_having_text_should_be_clicked(String accessName, String value,String data) throws Throwable {
        // Write code here that turns the phrase above into concrete actions
        String data_value =retriveTestdata.YamlFileReader().get(data).get(value);
        assertionObj.clickRepeaterElementText(accessName,data_value);
    }
